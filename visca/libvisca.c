...
#ifdef DEBUG_ENABLED
    fprintf(stderr, "_VISCA_get_reply: error iface->type[%u]\n", iface->type);
#endif
    return VISCA_FAILURE;
}
...
VISCA_set_address(VISCAInterface_t *iface, int *camera_num)
{
    ...
    else
    {
        ...
        {
#ifdef DEBUG_ENABLED
            fprintf(stderr, "VISCA_set_address: error not multiple of 4. iface->bytes[%d]\n", iface->bytes);
#endif
            return VISCA_FAILURE;
        }
        else
        {
            *camera_num = iface->ibuf[iface->bytes - 2] - 1;
            ...
            {
#ifdef DEBUG_ENABLED
                fprintf(stderr, "VISCA_set_address: error #camera[%d]\n", *camera_num);
#endif
                return VISCA_FAILURE;
            }
            else {
...
VISCA_get_camera_info(VISCAInterface_t *iface, VISCACamera_t *camera)
{
    ...
    {
#ifdef DEBUG_ENABLED
        fprintf(stderr, "VISCA_get_camera_info: we expect 10 bytes as answer. %d bytes received\n", iface->bytes);
#endif
        return VISCA_FAILURE;
    }
    else
...
VISCA_API uint32_t
VISCA_set_pantilt_absolute_position(VISCAInterface_t *iface, VISCACamera_t *camera, uint32_t pan_speed, uint32_t tilt_speed, int pan_position,
        int tilt_position)
{
    VISCAPacket_t packet;
...
VISCA_API uint32_t
VISCA_set_pantilt_relative_position(VISCAInterface_t *iface, VISCACamera_t *camera, uint32_t pan_speed, uint32_t tilt_speed, int pan_position,
        int tilt_position)
{
    VISCAPacket_t packet;
